...
@Component({
  selector: 'app-sigdoc-tramitar-documento-modal',
  templateUrl: './tramitar-documento.component.html',
  styleUrls: ['./tramitar-documento.component.css'],
})
export class TramitarDocumentoComponent implements OnInit, OnChanges {
  ...
  buscarDirecaoOrgao() {
    ...
    this.direcaoOuOrgaoService
      ...
      .subscribe((response: any) => {
        if (this.filtro.pessoajuridica_id) {
          this.pessoaJuridicas = [response];
          ...
          .subscribe({
            next: (response: any) => {
              if (response.length >= 4) { 
                const primeiroItem = response[0];
                const quartoItem = response[3];
      
                this.procedenciaCorrespondencia = [
                  {
                    id: quartoItem.id,
                    text: quartoItem.nome.toString().toUpperCase(),
                  },
                  {
                    id: primeiroItem.id,
                    text: primeiroItem.nome.toString().toUpperCase(),
  ...
  private editForm() {
    if (this.documento) {
      ...
      this.simpleForm.patchValue({
        ...
        referenciaDoc: this.documento.referenciaDoc, // Adicionado
        tipo_natureza_id: this.documento.tipo_natureza_id, // Adicionado
        tipo_correspondencia_id: this.documento.tipo_correspondencia_id // Adicionado
      });
    }
  }  
  ...
  public selecionarProcedencia($event: any): void {
    if ($event == 1) {
      ...
      this.simpleForm.get('tipo_orgao')?.disable();
      this.simpleForm.get('tipo_orgao')?.setValue(null);
      this.tituloDestino = 'Secretariado do Orgão/Comando Provincial';
      this.buscarDirecaoOrgao();
    } else {
      return;
    }
