...
@Component({
  selector: 'app-sigdoc-enviar-departameto-modal',
...
export class RegistarOuEditarComponent implements OnInit, OnChanges {
  ...
  @Input() public EnviarDepartamento: any = null
  ...
  constructor(
    ...
    private departamentoService: DepartementoService,
      ...
      .subscribe({
        next: (response: any) => {
          this.procedenciaCorrespondencia = response
            .filter((item: any) => item.nome.toString().toUpperCase() === 'DEPARTAMENTO')
            .map((item: any) => ({
              id: item.id,
              text: item.nome.toString().toUpperCase(),
            }));
          
          // Encontra o item 'DEPARTAMENTO'
          const departamento = this.procedenciaCorrespondencia.find(item => item.text === 'DEPARTAMENTO');
          
          if (departamento) {
            // Define 'DEPARTAMENTO' como valor padrão
            //this.simpleForm.get('procedencia_correspondencia_id').setValue(departamento.id);
          }
        },
      });
  }
  ...
  private criarForm() {
    this.simpleForm = this.fb.group({
      correspondencia_id: [this.idDocumentoClick],
  ...
  get idDocumentoClick() {
    return this.EnviarDepartamento?.id
  }
  ...
  public onSubmit(): void {
    ...
    const type = this.documento
      ? this.departamentoService.editar(data, this.documento.id)
      : this.departamentoService.registar(data);
    type
      .pipe(
        finalize((): void => {
  ...
  private get getFormData() {
    ...
    formData.append('correspondencia_id', this.simpleForm.get('correspondencia_id')?.value);
  ...
  setEnviarDepartamento(item: any) {
