...
import { TratamentoCorrespondenciaService } from '@resources/modules/sigdoc/core/service/Tratamento-correspondencia.service';
...
export class RegistarOuEditarComponent implements OnInit, OnChanges {
  ...
  public onSubmit() {
    ...
    const type = this.getId
      ? this.tratamentoCorrespondenciaService.editar(formData, this.getId)
      : this.tratamentoCorrespondenciaService.registar(formData);
  ...
  public get dataForm() {
    ...
    data.append(
      'sigdoc_correspondencia_id',
      this.simpleForm.get('sigdoc_correspondencia_id')?.value
    );
  ...
  public reiniciarFormulario() {
    ...
    this.simpleForm.patchValue({
      ...
      sigdoc_correspondencia_id: this.getCorrespondenciaId,
    });
    $('file-anexo').val('');
