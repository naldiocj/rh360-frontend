...
@Component({
  selector: 'app-agente-por-ordem',
  templateUrl: './agente-por-ordem.component.html',
  styleUrls: ['./agente-por-ordem.component.css'],
})
export class AgentePorOrdemComponent implements OnInit, OnDestroy {
  public pagination = new Pagination();
  public totalAgente: number = 0;
  public numero: any | null = null;
  public numero_: any | null = null;
  private destroy$: Subject<void>;
  public agente: any | null = null;
  ...
  private buscarProvimento() {
    this.provimentoService
      .listarUmPorOrdem({ numero: this.getNumero })
      .pipe(
        finalize(() => {
  ...
  private buscarPromocaoEmTempo() {
    const options = { ...this.filtro, numero: this.getNumero };
    this.isLoading = true;
    this.provimentoService
      ...
      .subscribe((response: any) => {
        ...
        this.totalAgente = response.meta?.total;
      });
  }
  ...
  public get getMaisAgentes() {
    return this.totalAgente > 1;
  }

  visualizar() {
    // this.provimentoService.listarUmPorOrdem({numero: this.getNumero}).pipe().subscribe({
    //   next: (response: any) => {
    //     this.numero = response;
    //   }
    // })
  ...
  public setVisualizar(item: any) {
    const options = { pessoaId: item?.id, numero: item.ordem_descricao };
    this.provimentoService
      .listar_promocao_emTempo(options)
      .pipe(
        finalize(() => {
