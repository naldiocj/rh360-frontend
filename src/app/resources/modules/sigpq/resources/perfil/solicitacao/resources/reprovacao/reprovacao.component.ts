...
@Component({
  selector: 'app-pa-reprovacao',
  templateUrl: './reprovacao.component.html',
  styleUrls: ['./reprovacao.component.css'],
})
export class ReprovacaoComponent implements OnInit {
  ...
  public onSubmit() {
    ...
    if (this.isPersonalizado) {
      this.tratamentoPdfService
        .registar(this.relatorioForm.value)
        .pipe(
          finalize((): void => {
        ...
        .subscribe({
          next: (res: any) => {
            this.reprovar(this.getSolicitacaoId);
          },
          error: (err: any) => {
            console.error(err);
          },
  ...
  private reprovar(id: any) {
    this.carregando = false;
    this.tratarSolicitacaoService
      .reprovar(id)
      .pipe(
        finalize((): void => {
          this.carregando = false;
      ...
      .subscribe({
        next: (res: any) => {
          this.modalService.fechar('btn-close-reprovacao');
