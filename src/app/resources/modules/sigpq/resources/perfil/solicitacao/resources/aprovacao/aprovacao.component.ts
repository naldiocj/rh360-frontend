...
@Component({
  selector: 'app-pa-aprovacao',
  templateUrl: './aprovacao.component.html',
  styleUrls: ['./aprovacao.component.css'],
})
export class AprovacaoComponent implements OnChanges, OnInit {
  ...
  public onSubmit() {
    ...
    if (this.isPersonalizado) {
      this.tratamentoPdfService.registar(this.relatorioForm.value).pipe(
        ...
        next: (res: any) => {
          this.aprovar(this.getSolicitacaoId)
        }, error: (err: any) => {
          console.error(err)
        }
      })
  ...
  private aprovar(id: any) {
    this.carregando = false;
    this.tratarSolicitacaoService.aprovar(id).pipe(
      finalize((): void => {
        this.carregando = false;
      })
    ).subscribe({
      next: (res: any) => {
        this.modalService.fechar('btn-close-aprovar')
        this.onTratamento.emit({ tratado: true })
        this.resetForm()
      }, error: (res: any) => {
        console.error(res)
      }
