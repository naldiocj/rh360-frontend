...
@Component({
  selector: 'app-sigpq-mobilidade-pdf',
  templateUrl: './pdf-mobilidade.component.html',
  styleUrls: ['./pdf-mobilidade.component.css']
})
export class PdfMobilidadeComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['listarPDF'].currentValue != changes['listarPDF'].previousValue) {
  ...
  generatePDF(item: any) {
    ...
    const texto1 = `Havendo a necessidade de retomar ao seu órgão de colocação, após cessar a sua missão na comissão de recolha de dados biométricos para emissão do novo cartão de identidade, marcha desta Direcção, para a ${item.orgao?.nome}/PNA, ${item.genero == 'M' ? 'o Senhor' : 'a Senhora'} ${item.patente_nome} ${item.nip ? ', Nip I ' + item.nip : ''}, ${item.nome_completo} ${item.apelido}`;
    const textoQuebrado1 = PDF.splitTextToSize(texto1, PDF.internal.pageSize.width - 20);
    ...
    const textoQuebrado2 = PDF.splitTextToSize(texto2, PDF.internal.pageSize.width - 20);
    ...
    PDF.setFontSize(8)
    y = 282;
    PDF.text(`Av. 4 de Fevereiro, nº. 206, Marginal de Luanda, 1º andar, edificio sede da PNA`, 10, y);
    y += 4;
    ...
    return
    html2canvas(this.invoiceElement.nativeElement, { scale: 1 }).then((canvas) => {
  ...
  urlPdfMobilidade(chave: any) {
    ...
    this.mobilidadeService.pdfEmGrupo({}).pipe(
      finalize(() => {
        // this.isLoading = false;
      })
    ).subscribe((response) => {
