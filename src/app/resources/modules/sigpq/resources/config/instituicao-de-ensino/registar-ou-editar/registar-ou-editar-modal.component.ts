...
export class RegistarOuEditarModalComponent implements OnInit, OnChanges {
  ...
  onSubmit() {
    ...
    const type = this.buscarId ? this.instrituicaoDeEnsinoService.editar(this.simpleForm.value, this.buscarId) : this.instrituicaoDeEnsinoService.registar(this.simpleForm.value)
    type.pipe(
      finalize(() => {
