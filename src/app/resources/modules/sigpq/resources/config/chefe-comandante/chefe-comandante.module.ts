import { NgModule } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ChefeComandanteRoutingModule } from './chefe-comandante-routing.module';
import { ListarComponent } from './listar/listar.component';
import { RegistarOuEditarComponent } from './registar-ou-editar/registar-ou-editar.component';
import { FormsModule, ReactiveFormsModule } from '@angular/forms';
import { NgxPaginationModule } from 'ngx-pagination';
import { NgSelect2Module } from 'ng-select2';
import { SharedComponentsModule } from '@resources/modules/pa/shared/shared-components/shared-components.module';
import { ComponentsModule } from '@resources/modules/sigpq/shared/components/components.module';
import { LoadingPageModule } from '@shared/components/loading-page.module';
import { ListarAgentesComponent } from './listar-agentes/listar-agentes.component';
import { ChefiaHistoricoComponent } from './chefia-historico/chefia-historico.component';



@NgModule({
  declarations: [
    ListarComponent,
    RegistarOuEditarComponent,
    ListarAgentesComponent,
    ChefiaHistoricoComponent
  ],
  imports: [
    CommonModule,
    ChefeComandanteRoutingModule,
    FormsModule,
    ReactiveFormsModule,
    NgxPaginationModule,
    NgSelect2Module,
    SharedComponentsModule,
    ComponentsModule,
    LoadingPageModule
  ]
})
export class ChefeComandanteModule { }
