 ...
 public options: any = {
  ...
  createForm() {
    this.simpleForm = this.fb.group({
      ...
      aceder_departamento: [false],
      aceder_seccao: [false],
      aceder_brigada: [false],
    });

    if (this.idUtilizador) {
      this.editForm()

    }else this.ajustarAcesso('direccao');

  }

  ajustarAcesso(acesso:string)
  {
    this.acesso=acesso;
    console.log("Acesso escolhido:",acesso)
  }

  mostrarMensagem(colocacao:string='Direcção')
  {
    const frase:string=`Deseja conceder acesso somente aos agentes ${colocacao} deste utilizador?`
    return frase
  }
  async editForm() {
    if (this.utilizador) {
      ...
      this.simpleForm.patchValue({
        ...
        aceder_departamento: this.utilizador.aceder_departamento,
        aceder_seccao: this.utilizador.aceder_seccao,
        aceder_brigada: this.utilizador.aceder_brigada,
      });

      await this.verificarQualAcessoOUtilizadorTem(
        this.utilizador.aceder_departamento,
        this.utilizador.aceder_seccao,
        this.utilizador.aceder_brigada
      )
  ...
  async verificarQualAcessoOUtilizadorTem(departamento:any,seccao:any,brigada:any)
  {
    console.log("Dados recebidos:",departamento,seccao,brigada)
    let acesso:string='direccao';
   if(this.temValorValidoEverdadeiro(departamento))  acesso='departamento';
   else if(this.temValorValidoEverdadeiro(seccao))   acesso='seccao';
   else if(this.temValorValidoEverdadeiro(brigada))acesso='brigada';
    this.ajustarAcesso(acesso);

    console.log("SELECIONADO ATUALMENTE:",this.acesso)

  }

  temValorValidoEverdadeiro(value: any): boolean {
    const result:boolean= value !== null && value !== undefined && value !== '';
    if(result)return Number(value)==1;
    return result
  }
  ...
  onSubmit() {
    ...
    this.permitirQueTenhaSomenteUmAcessoOuNenhum()
    ...
    type.pipe(
      ...
      async permitirQueTenhaSomenteUmAcessoOuNenhum() {
        const acessos:any = {
          departamento: { aceder_departamento: true, aceder_seccao: false, aceder_brigada: false },
          seccao: { aceder_departamento: false, aceder_seccao: true, aceder_brigada: false },
          brigada: { aceder_departamento: false, aceder_seccao: false, aceder_brigada: true }
        };

        this.simpleForm.patchValue(acessos[this.acesso] || { aceder_departamento: false, aceder_seccao: false, aceder_brigada: false });
      }
