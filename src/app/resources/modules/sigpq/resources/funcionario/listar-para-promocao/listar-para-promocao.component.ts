...
@Component({
  selector: 'app-listar-para-promocao',
  templateUrl: './listar-para-promocao.component.html',
  styleUrls: ['./listar-para-promocao.component.css']
})
export class ListarParaPromocaoComponent implements OnInit {
  ...
  constructor(private propostaProvimentoService: PropostaProvimentoService,
  ...
  buscarPromocaoEmTempo() {
    ...
    this.propostaProvimentoService.listar_promocao_emTempo(options).pipe(
      ...
      console.log("Funcionarios retornandos da API:",response.data)
  ...
  atribuirNullCheckBox(id: any) {
    ...
    this.filtro[id] = 'null';
    const element: HTMLInputElement = document.querySelector(`#${id}`) as HTMLInputElement
    if (element) {
  ...
  selecionarAgenteParaMobilidade(item: any) {
    // Verifica se já existem agentes no conjunto
    if (this.agentesSelecionados.length > 0) {
      const patenteExistente = this.agentesSelecionados[0]?.patente_nome;

      if (item.patente_nome !== patenteExistente) {
        alert('Todos os agentes selecionados devem ter a mesma patente.');
