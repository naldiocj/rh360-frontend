...
@Component({
  selector: 'sigpq-historico-policial',
  templateUrl: './historico-policial.component.html',
  styleUrls: ['./historico-policial.component.css'],
})
export class HistoricoPolicialComponent implements OnInit {
  ...
  private isEdittingMode() {
    return isNaN(this.params.getId) ||
      this.params.getId === '' ||
      this.params.getId === null
      ? false
      : true;
  }
  ...
  private buscarHistoricoDeEmpregos() {
    ...
    this.outrosEmpregosFuncionariosService
      .listar({
  ...
  private buscarTipoCurso(tipo: any = null): void {
    ...
    this.listaEmpresaservice
      .listarTodos(opcoes)
      .pipe(finalize((): void => {}))
      .subscribe((response: any): void => {
        this.listaEmpresas = response.map((item: any) => ({
          id: item.id,
          text: item.nome,
        }));
      });
  ...
  public onSubmit() {
    ...
    const type = this.buscarId
      ? this.outrosEmpregosFuncionariosService.editar(dados, this.buscarId)
      : this.outrosEmpregosFuncionariosService.registar(dados);
  ...
  private get getDado() {
    ...
    formData.append(
      'tipo_de_contrato',
      this.getTipoContratoTexto(this.simpleForm.value.tipo_de_contrato) ??
        'Efectivo'
    );
  ...
  getTipoContratoId(texto: string): string | undefined {
    const tipo = this.tiposContrato.find(
      (tipo) => tipo.text.toLowerCase() === texto.toLowerCase()
    );
