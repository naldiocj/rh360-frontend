...
interface typeAtividade {
  name: string;
  sigpq_excluir_estado_reforma_id: {
    reformado: string | number | null;
    falecido: string | number | null;
    demitido: string | number | null;
  };
}

@Component({
  selector: 'app-fora-de-efetividade',
  templateUrl: './fora-de-efetividade.component.html',
  styleUrls: ['./fora-de-efetividade.component.css'],
})
export class ForaDeEfetividadeComponent implements OnInit, OnDestroy {
  public estados = [
    {
      cor: 'rgb(254, 176, 25)',
      texto: 'Em licença',
  ...
  public tituloSituacao: string = 'Fora de Actividade';
  ...
  ngOnInit() {
    ...
    this.tituloSituacao =
      this.type.name == 'fora-de-atividade'
        ? 'Fora de Actividades'
        : this.type.name == 'Inactivo'
        ? 'Inactivos'
        : 'em Inatividade Temporária';
    this.buscarRegimes();
  ...
  private buscarSituacaoEstados() {
    this.situacaoEstadoService
      ...
      .subscribe({
        next: (response: any) => {
          let aux = response.filter(
            (item: any) =>
              item.nome !== 'Efectividade' && item.nome !== 'Efetividade'
          );

          // Verifica o tipo e aplica o filtro correspondente
          if (this.type.name === 'fora-de-atividade') {
            aux = aux.filter(
              (item: any) =>
                item.nome !== 'Inactividade Temporária' &&
                item.nome !== 'Inactividade Temporaria'
            );
            aux = aux.filter(
              (item: any) =>
                item.nome !== 'Inactivo' && item.nome !== 'Inactivo'
            );
          } else if (this.type.name === 'inatividade-temporaria') {
            aux = aux.filter(
              (item: any) =>
                item.nome !== 'Fora de Actividade' &&
                item.nome !== 'Fora de Actividade'
            );
            aux = aux.filter(
              (item: any) =>
            ...
            aux = aux.filter(
              (item: any) =>
                item.nome !== 'Comissão de Normal de Serviço' &&
                item.nome !== 'Comissão de Normal de Serviço'
            );
            aux = aux.filter(
              (item: any) =>
                item.nome !== 'Comissão Especial de Serviço' &&
                item.nome !== 'Comissão Especial de Serviço'
            );
          } else {
            aux = aux.filter(
  ...
  validarAlgunsFiltros() {
    ...
    if (this.filtro.estadoId) {
      if (this.filtro.situacaoId != '6') {
        this.filtro.sigpq_excluir_estado_reforma_id = [
          this.type.sigpq_excluir_estado_reforma_id.demitido,
          this.type.sigpq_excluir_estado_reforma_id.falecido,
          this.type.sigpq_excluir_estado_reforma_id.reformado,
        ];
      } else {
        delete this.filtro.sigpq_excluir_estado_reforma_id;
      }
    }
  }
  buscarFuncionario() {
