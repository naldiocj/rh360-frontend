...
import { calcularDiasUteis } from '../../../core/helper/calcularDiasUteis';
...
export class AgendarLicencaComponent implements OnInit, AfterViewInit {
  @ViewChild('licenca_disciplinar_pa', { static: false })
  licenca_disciplinarComponent!: GerarDocumentoPdfParaLicencaComponent;
  constructor(
    private relatorioService: RelatorioLicencaService,
    private tipoLicencaFuncionario: TipoLicencaParaFuncionarioService,
    private licencaparaFuncionario: LicencaParaFuncionarioService,
    private feriadosService: FeriadosService,
    private formatDate: FormatarDataHelper
  ) {}

  fileUrl: any = null;
  mensagem_caso_atinja_limite: string =
    'Infelizmente, excedeste ou não há mais dias disponíveis para selecionar nesta licença.';
  ...
  @Output() devolverOsFiltrosRecebidos = new EventEmitter<any>();
  ...
  notificarPai() {
    ...
    this.devolverOsFiltrosRecebidos.emit(this.filtro_do_componente_pai);
  }

  feriasevents: any;
  buscarFeriadosEAplicarNosEventos() {
    this.feriadosService
      .listar({})
      .pipe()
      .subscribe((response: any) => {
        ...
        eventos.forEach((element: any) => {
          const newEvent = {
            ...
            extendedProps: {
              description:
                element.licenca_aplicada == 1 ? 'feriado' : 'indisponivel',
  ...
  async buscarDias() {
    this.licencaparaFuncionario
      ...
      .subscribe((response: any) => {
        ...
        eventos.forEach((element: any) => {
          const newEvent = {
            title: element.motivo_rejeicao
              ? element.motivo_rejeicao
              : element.descricao,
  ...
  planejarLicenca() {
    ...
    if (!this.verificarSeEaLicenca('Disciplinar'))
      Eventos = Eventos.filter(
        (event: any) =>
          event.extendedProps.description !== 'pendente' &&
          event.extendedProps.description !== 'aprovado' &&
          event.extendedProps.description !== 'rejeitado' &&
          event.extendedProps.description !== 'feriado' &&
          event.extendedProps.description !== 'indisponivel'
      );
  ...
  @Input() informacao:
    ...
    | 'visualizar'
    | string = 'planificar';
  @Input() local: 'licenca' | 'falta' | 'feria' | string = 'licenca';

  public licencaSelecionada: any = null;
  @Input() anoAtual!: number;
  @Input() mesAtual!: number;
  @Input() filtro_do_componente_pai!: any;

  public abertoPelaPrimeiraVez: boolean = true;
  ...
  buscarNomeDaLicenca(): string {
    console.log(this.licencaSelecionada);
    // if(this.ehNulo(this.licencaSelecionada)) return ''
    const aux = this.licencas.find(
      (item: any) => item.id == this.licencaSelecionada
    );
    return aux.nome;
  }

  ehNulo(valor: any): boolean {
    return (
      valor === null || valor === undefined || valor === 'null' || valor === ''
    );
  }

  buscarTipoLicencasParaFuncionarios() {
    this.tipoLicencaFuncionario
      .listarTodos({})
      .pipe()
      .subscribe({
        next: (response: any) => {
  ...
  verificarSeEaLicenca(_licenca: string): boolean {
    const licenca = this.licencas.find(
      (l) => l.nome.toUpperCase() === _licenca.toUpperCase()
    );
  ...
  return month === selectedMonth;
    ...
    const diasMaximos =
      this.dias_maximosLicenca +
      this.diasAdicionais -
      (this.pegarDiferencaEventos() + this.dispensas_gastas);
    let eventos_selecionados_em_ferias: IInputDay[] = [];
    if (this.verificarSeEaLicenca('Disciplinar')) {
      await this.calcularDiasUteis
        .calcularDias(
          selectedMonth,
          this.anoAtual,
          diasMaximos,
    ...
    formData.append(
      ...
      JSON.stringify(
        licenca_disciplinar
          ? eventos_selecionados_em_ferias
          : eventos_selecionados
      ) // Serializar o array para enviar como string
    );
  ...
  enviarMaisInformacoes(local: 'motivo' | 'resposta' | 'resposta-maxima') {
    ...
    // Fazer o registro
    this.licencaparaFuncionario
      .enviar_detalhe_para_licenca_ano_mes(formData)
      .pipe()
      .subscribe({
        next: (response: any) => {
  ...
  inserirDiasPendenteNoAuxERemoverNoEvents(eventos: any) {
    eventos.forEach((evento: any) => {
      ...
      if (index !== -1) {
        ...
        if (
          eventoCorrespondente.extendedProps &&
          eventoCorrespondente.extendedProps['description'] == 'aguardando'
        ) {
        ...
        //Especificamente na licença Disciplinar
    ...
    eventContent: (arg) => {
      ...
      titleEl.innerHTML = [
        ...
        ? arg.event.title.length > 4
          ? arg.event.title.toUpperCase() == 'Dia selecionado'.toUpperCase()
            ? ''
            : arg.event.title.substring(0, 18) + '...'
          : arg.event.title
        : '';
  ...
  onDatesSet(info: any) {
    ...
    if (this.abertoPelaPrimeiraVez) {
      this.abertoPelaPrimeiraVez = false;

      if (this.anoAtual == 0) this.anoAtual = dataAtualVisivel.getFullYear();
      if (this.mesAtual == -1) this.mesAtual = dataAtualVisivel.getMonth();
  ...
  handleDateClick(arg: any) {
    ...
    if (!this.possuiErroNumaAtualizacaoDaBd) {
      ...
      if (
        !this.verificaSeEssaDataEstaNaBaseDeDados(
          new Date(this.selectedDate)
        ) &&
        !this.isWeekend(this.selectedDate)
      ) {
        ...
        this.verificarEssaDataComStatusDe(
          new Date(this.selectedDate),
          'feriado'
        )
      ) {
        ...
        if (this.informacao == 'planificar')
          this.showModal('confirmModalRegeted'); ///Analisar o nivel de permissão do usuário
      } else if (
        this.verificarEssaDataComStatusDe(
          new Date(this.selectedDate),
          'pendente'
        ...
        const event = this._auxevents.find(
          (event) =>
            event.date === this.formatDate.formatDate(this.selectedDate)
        ); // Verifica se há um evento na data selecionada
  ...
  deleteEvent() {
    if (this.selectedDate) {
      this.events = this.events.filter(
        (event) => event.date !== this.formatDate.formatDate(this.selectedDate)
      );
      this.updateEvents();
  ...
  confirmToDeleteDataFromDB() {
    if (this.selectedDate) {
      ...
      const eventoRejeitado = this._auxevents.find(
        (event) => event.date === formattedDate
      );

      if (eventoRejeitado?.extendedProps?.['identificador']) {
        this.licencaparaFuncionario
          .eliminar(eventoRejeitado.extendedProps['identificador'])
          .pipe()
          .subscribe((response: any) => {
            this._auxevents = this._auxevents.filter(
              (event) => event.date !== formattedDate
            );
            this.updateEvents();
          });
  ...
  verificarEssaDataComStatusDe(
    data: Date,
    status: 'feriado' | 'aprovado' | 'rejeitado' | 'pendente' | 'indisponivel'
  ): boolean {
    // Formata a data para o formato YYYY-MM-DD
    const formattedDate = this.formatDate.formatDate(data);
    const finded = this._auxevents.find(
      (event: EventInput) => event.date == formattedDate
    );
    return !!finded && finded.extendedProps?.['description'] == status;
  }

  verificaSeIntervaloDatasEstaNaBaseDeDadosComStatusDe(
    data: Date,
    _data: Date,
    status: 'feriado' | 'aprovado' | 'rejeitado' | 'pendente' | 'indisponivel'
  ): boolean {
  ...
  aproveEvent() {
    ...
    const eventosCorrespondentes = this._auxevents.filter((event: any) =>
      diasFormatados.includes(event.date)
    );
  ...
  handleSelect(info: any) {
    ...
    if (!this.possuiErroNumaAtualizacaoDaBd) {
      ...
      if (
        !this.verificaSeIntervaloDatasEstaNaBaseDeDados(
          this.selectedStartDate,
          this.selectedEndDate
        )
      ) {
        if (dayDifference > 1) {
          ...
          this.selecionouDiasQueJaExisteNoCalendario =
            this.verificarSeDiasSelecionadosContemNoEventos();
          if (this.selecionouDiasQueJaExisteNoCalendario) {
            if (this.informacao == 'planificar') this.showModal('confirmModal');
          } else {
            if (
              this.verificarSeTemDiasDisponiveisSelecao() &&
              this.informacao == 'planificar'
            )
      ...
      if (dayDifference > 1) {
        // Mostra o modal de confirmação se a seleção for maior que 1 dia
        if (
          this.verificaSeIntervaloDatasEstaNaBaseDeDadosComStatusDe(
            this.selectedStartDate,
            this.selectedEndDate,
            'aprovado'
          ) &&
  ...
  verificarSeDiasSelecionadosContemNoEventos() {
    let selectedDays = this.generateDays(
      this.selectedStartDate,
      this.selectedEndDate,
      false
    );
  ...
  verificarSeTemDiasDisponiveisSelecao(
    incluirFimDeSemana: boolean = false
  ): boolean {
    let selectedDays = this.generateDays(
      this.selectedStartDate,
      this.selectedEndDate,
  ...
  calcularDiasRestantes(): number {
    if (this.calendarOptions.events) {
      const filtros = {
        situacao: ['pendente', 'aprovado', 'aguardando'],
        ano: [this.anoAtual],
      };
      const totalDiasSelecionados = this.calcularUtilizacoes(
        this.calendarOptions.events,
        filtros
      );
      // Retorna o valor absoluto para evitar números negativos
      return totalDiasSelecionados.length > 0
        ? Math.abs(totalDiasSelecionados[0].diasUsados)
        : 0;
    }
    return 0;
  }
  ...
  async saberQuantasDispensas(): Promise<number> {
    ...
    try {
      const response = await this.licencaparaFuncionario
        .listar(_filtro)
        .toPromise();
  ...
  confirmSelection(incluirDiaFim: boolean = false) {
    if (this.selectedStartDate && this.selectedEndDate) {
      ...
      selectedDays.forEach((day) => {
        ...
        if (
          !this.verificaSeEssaDataEstaNaBaseDeDados(day) &&
          !this.verificaSeEssaDataEstaNoEvento(day)
        )
          this.events = [
            ...this.events,
            {
  ...
  async onLicencaSelecionada(event: any) {
    if (event !== null) this.licencaSelecionada = event;
    ...
    if (licenca) {
      ...
      if (licenca.dias_maximos) {
        ...
        this.dias_maximosLicenca = licenca.dias_maximos
          ? licenca.dias_maximos
          : 0;
    ...
    this.filtro.pessoajuridica_id = this.agenteSelecionado?.orgao.id; //this.agenteSelecionado?.orgao.id;
    if (event) this.buscarDias();

    if (this.verificarSeEaLicenca('Disciplinar'))
      this.dispensas_gastas = await this.saberQuantasDispensas();
  ...
  calcularLimitesAnuais(diasSolicitados: Date[]) {
    ...
    if (this.verificarSeEaLicenca('Disciplinar'))
      licenca = {
        dia_maximo: _licenca.dias_maximos + this.dispensas_gastas,
        limite_anual: _licenca.limite_anual,
        utilizacoes: utilizadoes,
      };
    else
      licenca = {
        dia_maximo: _licenca.dias_maximos,
        limite_anual: _licenca.limite_anual,
        utilizacoes: utilizadoes,
      };
    const { dia_maximo, limite_anual, utilizacoes } = licenca;

    const diasNoAnoAtual = diasSolicitados.filter(
      (dia) => dia.getFullYear() === anoAtual
    );
    const usoAtual = utilizacoes.find(
      (uso: Utilizacao) => uso.ano == anoAtual
    ) || { ano: anoAtual, diasUsados: 0 };
    const daisRestantes = dia_maximo * limite_anual;
    const diasRestantesNoAno =
      dia_maximo * limite_anual - (usoAtual.diasUsados + diasNoAnoAtual.length);

    if (diasRestantesNoAno == 0) {
      return {
        valido: true,
        mensagem: `A solicitação é permitido, pois é o ultimo dia do limite máximo de ${daisRestantes} dias neste ano. ${anoAtual}`,
      };
    } else if (diasRestantesNoAno > 0) {
      return {
        valido: true,
        mensagem: `A solicitação é permitido, pois não excede o limite máximo de ${daisRestantes} dias neste ano. ${anoAtual}`,
      };
    } else {
      return {
        valido: false,
        mensagem: `A solicitação interrompe o limite máximo de ${daisRestantes} dias neste ano. ${anoAtual}`,
      };
    }
  }
      ...
      return Object.entries(filtros).every(([campo, valores]) => {
        ...
        if (campo === 'situacao' && Array.isArray(valores)) {
          return valores.includes(
            item.extendedProps.description?.trim().toLowerCase()
          );
        }
...
/*const formData = new FormData();
  ...
  formData.append('mes', (selectedMonth).toString());
  formData.append('ano', this.anoAtual.toString());
  formData.append('feriados',JSON.stringify(filteredFeriados));
  formData.append('dias_maximos',((this.dias_maximosLicenca+this.diasAdicionais) - (this.pegarDiferencaEventos()+this.dispensas_gastas)).toString())
  formData.append(
    'dias_selecionados',
    JSON.stringify(eventos_selecionados) // Serializar o array para enviar como string
  );
  formData.append(
    'dias_selecionados_para_nao_fazer_parte',
    JSON.stringify(dias_selecionados_para_nao_fazer_parte) // Serializar o array para enviar como string
  );
  formData.append('pessoafisica_id', this.agenteSelecionado.id);
  formData.append('situacao', 'pendente');
  formData.append('motivo', this.mais_informacoes);
