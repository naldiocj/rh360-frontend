...
@Component({
  selector: 'app-analisar-licenca',
  templateUrl: './analisar-licenca.component.html',
  styleUrls: ['./analisar-licenca.component.css']
})
export class AnalisarLicencaComponent implements OnInit,OnDestroy {
  ...
  filtro = {
    ...
    situacao_do_dia_da_licenca:'null',
    estadoId: 'null',
    orgaoId: 'null',
    genero: 'null',
    search: "",
  ...
  ngOnDestroy(): void {
    sessionStorage.setItem("buscaUsuarioAnalisarLicenca", JSON.stringify(this.filtro));
  }

  ngOnInit() {
    this.buscarRegimes()
    // this.buscarClasses()
    this.carregarAsVariaveisUteis()
  ...
  private carregarAsVariaveisUteis()
  {
    this.informacao = this.route.snapshot.data['informacao']|| 'planificar';
    if(this.informacao=='visualizar' || this.informacao=='rejeitar'){
      this.filtro.situacao_do_dia_da_licenca='aprovado'
    }else if(this.informacao=='analisar'){
      this.filtro.situacao_do_dia_da_licenca='pendente'
    }
  ...
  atualizarFiltors(filtros_vindo_do_filho:any)
  {
    //console.log(filtros_vindo_do_filho)
  ...
  analisarFiltroNoStorageEFazerAtualizacao()
  {
    ...
    //const buscaUsuario = sessionStorage.getItem("buscaUsuarioAnalisarLicenca");
    ...
    this.validarLicencaDisciplinar()
  }
  ...
  buscarFuncionario() {
    ...
    this.isLoading = true;
    if(this.local=='feria')this.filtro.licenca_disciplinar='true'
    this.licencaparaFuncionario.listarTodasLicencasComPessoas(this.filtro).pipe(
      finalize(() => {
        this.isLoading = false;
      })
    ).subscribe((response:any) => {
      this.funcionarios = response.data;
      console.log("FUNCIONARIOS ENCONTRADOS NA LICENCA:",response)
  ...
  eliminar(id: number) {
    this.isLoading = true;
    this.licencaparaFuncionario.eliminar(id).pipe(
      finalize(() => {
        this.isLoading = false;
      })
    ).subscribe((response) => {
  ...
  async preencherAnos()
  {
      this.anosDisponiveis= await funcaoGenericaGerarAnos()
  }


  async preencherMesesEAno() {
    this.mesesDisponiveis=await funcaoGenericaGerarMeses();
    await this.preencherAnos()

    await this.buscarTipoLicencasParaFuncionarios()
  }
