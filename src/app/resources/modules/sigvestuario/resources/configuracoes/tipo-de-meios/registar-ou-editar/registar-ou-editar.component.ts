...
export class RegistarOuEditarComponent implements OnInit, OnChanges, OnDestroy{
  ...
  public tipo_de_meios_form!: FormGroup;
  ...
  validarTipoDePlanoForm(): void {
    this.tipo_de_meios_form = this.form_builder.group({
  ...
  preencherTipoDePlanoForm(): void {
    ...
    this.tipo_de_meios_form.patchValue({
  ...
  public async onSubmit() {
    if (this.tipo_de_meios_form.invalid) {
      return;
    }
    console.log('oi')

    const type = this.plano_id? this.tipo_de_meios_Service.editar(this.plano_id, this.tipo_de_meios_form.value) : this.tipo_de_meios_Service.registar(this.tipo_de_meios_form.value);
    await type.pipe(
      finalize(() => {})
    ).subscribe((response) => {
