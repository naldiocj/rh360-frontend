...
export class RegistarComponent implements OnInit, CanDeactivateComponent {
  ...
  public dados_do_funcionario_ou_efectivo_array: any;
  ...
  constructor(
    private planos_de_distribuicao_service: PlanoDeDistribuicaoService,
  ...
  private async inserirDadosNaApi(planificacao: any) {
    await this.planos_de_distribuicao_service.registar(planificacao).subscribe({
      next: (response) => {
  ...
  descartarPlano(id: any) {
    this.planos_de_distribuicao_service.eliminar(id).subscribe({
      next: (response) => {
        //this.buscarPlanos();
      },
      error: (err) => {
  ...
  public adicionarOsDadosDoEfectivoArray(dados: any) {
    this.dados_do_funcionario_ou_efectivo_array = dados;
