...
export class RegistarMeiosComponent implements OnInit, OnChanges {
  @Input() plano_id: any;
  @Input() dados_do_funcionario_ou_efectivo: any;
  @Input() dados_do_funcionario_ou_efectivo_array: any;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Verifica se plano_id mudou e não é o primeiro carregamento
    if (changes['dados_do_funcionario_ou_efectivo'] && !changes['dados_do_funcionario_ou_efectivo'].firstChange) {
      //this.carregarProdutos();
      console.log(this.dados_do_funcionario_ou_efectivo.id)
      this.validarprodutosPlanificadosForm();
      this.produtosPlanificadosForm.get('pessoajuridica_id')?.setValue(this.dados_do_funcionario_ou_efectivo.id)
      console.log(this.produtosPlanificadosForm.get('pessoajuridica_id')?.value)
      this.adicionarItem()

    }

    if (changes['dados_do_funcionario_ou_efectivo_array'] && !changes['dados_do_funcionario_ou_efectivo_array'].firstChange) {
      //this.carregarProdutos();
      console.log(this.dados_do_funcionario_ou_efectivo_array)
      this.validarprodutosPlanificadosForm();
      /* this.produtosPlanificadosForm.get('pessoajuridica_id')?.setValue(this.dados_do_funcionario_ou_efectivo.id)
      console.log(this.produtosPlanificadosForm.get('pessoajuridica_id')?.value) */
      this.adicionarItem()
  ...
  adicionarItem() {
    const novoItem = this.fb.group({
      ...
      prazo_uso: '',
      norma_pessoa_id: '',
      login_pessoajuridica_id: this.orgao_id,
      pessoajuridica_id: this.produtosPlanificadosForm.get('pessoajuridica_id')?.value,
      imagemUrl: [''],
      unidadesMedida: ['']
    });
  ...
  public adicionarTipoDeCarreiraAoPlano(id: any) {
    this.produtosPlanificadosForm.get('pessoajuridica_id')?.setValue(id);
    console.log(id)

    if (this.itens.length > 0) {
      this.itens.at(0)?.get('pessoajuridica_id')?.setValue(id);
    }
  ...
  vincularProdutosUsuarios() {
    const form_dados = this.produtosPlanificadosForm.value;
    const produtos = form_dados.itens; // Lista dos produtos no FormArray
    const usuariosSelecionados = this.dados_do_funcionario_ou_efectivo_array; // IDs dos usuários selecionados
    ...
    for(let usuario of usuariosSelecionados){
      ...
      for(let item of produtos){
        vinculos.push({
          pessoajuridica_id: usuario.id,
          meio_id: item.meio_id, // Ou outra propriedade que representa o ID do produto
    ...
    console.log('Lista de vínculos:', vinculos);
  
    // Aqui você pode enviar o "vinculos" para a API, salvar, etc
    this.percorrerObjectoItens(vinculos)
  }
  ...
  registarNaBaseDeDados(meio: any) {
    this.meios_plano_de_distribuicao_service.registar(meio).subscribe({
      next: (response) => {
  ...
  selecionarOrgaoOuComandoProvincial($event: any): void {
    ...
    if (!$event) {
      this.produtosPlanificadosForm.get('id_departamento_envio')?.disable()
      this.produtosPlanificadosForm.get('id_departamento_envio')?.setValue(null)
    } else {
      this.direcao_ou_orgao_service.listarTodos({
        ...
        .subscribe((response: any): void => {
          ...
          this.produtosPlanificadosForm.get('pessoajuridicas_id')?.enable()
  ...
  finalizar(){
    this.limparEstadoProcessamento.emit(true);
    this.router.navigate(['/piips/sigvest/planificacao/plano-de-distribuicao/individual/listar']);
  }
}