...
import { MeiosAtribuidosService } from '@resources/modules/sigvestuario/core/meios/meios-atribuidos.service';
...
export class ListarMeiosComponent implements OnInit {
  ...
  private async buscarProdutos() {
    ...
    this.filtro.pessoafisicas_id = plano_id

    await this.meios_atribuidos_service
      .listar(this.filtro)
      .pipe(
        finalize((): void => {
          this.isLoading = true;
  ...
  private async buscarBeneficiarios() {
    await this.beneficiarios_service.listarUm('', this.num)
