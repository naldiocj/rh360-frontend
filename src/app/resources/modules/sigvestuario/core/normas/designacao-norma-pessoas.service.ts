...
export class DesignacaoNormaPessoasService {
  private api: string = '/api/v1/sigvest';
  private base: string = this.api + '/norma_pessoas';
  ...
  public registar(designacao_de_norma_pessoas: any): Observable<any> {
    console.log(designacao_de_norma_pessoas)
    return this.httpApi.post(`${this.base}`, designacao_de_norma_pessoas).pipe(
      distinctUntilChanged(),
      debounceTime(500),
      map((response: Object) => {
        return Object(response).object
