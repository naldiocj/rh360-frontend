...
@Component({
  selector: 'app-listar-associacao-delituosa',
  templateUrl: './listar-associacao-delituosa.component.html',
  styleUrls: ['./listar-associacao-delituosa.component.css']
})
export class ListarAssociacaoDelituosaComponent implements OnInit {
  isLoading!: boolean;
  totalBase: any;
  pagination: any;
  associacaoDelituosos: any;
  ...
  buscarAssociacao() {
    this.isLoading = true;
    this.associarDelituoso.listarTodos({}).pipe(
      finalize(() => {
        this.isLoading = false;
      })
    ).subscribe((response: any) => {
      this.associacaoDelituosos = response;
