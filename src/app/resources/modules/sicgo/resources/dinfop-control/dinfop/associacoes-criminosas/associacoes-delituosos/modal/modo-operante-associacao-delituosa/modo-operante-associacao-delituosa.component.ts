...
import { DinfopAssociacaoDelituosaModoOperanteService } from '@resources/modules/sicgo/core/service/piquete/dinfop/dinfop_modo_operante/dinfop_modo_operante/dinfop-modo-operante.service';
import { Select2OptionData } from 'ng-select2';
import { Validators } from 'ngx-editor';
import { finalize } from 'rxjs';

@Component({
  selector: 'sicgo-dinfop-modo-operante-associacao-delituosa',
  templateUrl: './modo-operante-associacao-delituosa.component.html',
  styleUrls: ['./modo-operante-associacao-delituosa.component.css']
})
export class ModoOperanteAssociacaoDelituosaComponent implements OnInit {
  ...
  constructor(
    ...
    private modoOperanteS: DinfopAssociacaoDelituosaModoOperanteService) {
    this.registrationForm = this.fb.group({
      titulo: ['', [Validators.required, Validators.minLength(3)]],
      descricao: ['', [Validators.required, Validators.maxLength(255)]],
      data_hora_inicio: ['', [Validators.required, this.validarDataPresentePassada]],
...
getDateValidation() {
 ...
 // Validador para aceitar apenas data presente ou passada
 validarDataPresentePassada(control: any) {
  const agora = new Date();
  const inputData = new Date(control.value);
  return inputData <= agora ? null : { dataInvalida: true };
}

// Validador para aceitar apenas data futura
