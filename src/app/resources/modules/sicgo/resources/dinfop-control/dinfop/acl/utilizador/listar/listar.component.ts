...
import { Agent, ExpedienteService } from '@resources/modules/sicgo/core/service/piquete/dinfop/expediente/expediente.service';
...
@Component({
  selector: 'app-sicgo-perfis-listar-utilizador',
...
export class ListarPerfisComponent implements OnInit {
  ...
  @Output() AngentesSelected = new EventEmitter<any>();
  ...
  agents: Agent[] = []; // Preencher com dados reais (via API)
  ...
  confirmSelection() {
    if (this.selectedAgents.length > 0) {
      this.expedienteService.updateSelection(this.selectedAgents);
    }
  }
