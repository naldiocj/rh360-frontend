...
@Component({
  selector: 'sigpj-perfil-permission',
...
export class PermissionComponent implements OnInit {
  ...
  setPermission() {
    this.permission.listarRolePermission(this.getId())
      .subscribe((array_permission) => {

        //  console.log("permission existed", array_permission)
       // if(!array_permission || array_permission == null){ return}
        this.permissionExisted = array_permission
        for (var item of array_permission) {
  ...
  onSubmit() {
    ...
    for (let id1 = 0; id1 < this.positionDelete.length; id1++) {
      ...
      for (const existedPermission of this.permissionExisted) {
        if (keys.value && permission.name == existedPermission.name) {
          ...
          this.permission.deleteRolePermission(options)
            .subscribe(function () {
    ...
    this.permission.registarRolePermission(options).subscribe((evt)=>{this.router.navigate(['/piips/sigpj/admin/perfil/listagem'])})
  }
